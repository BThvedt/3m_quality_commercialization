<template>
  <PageBase>
    <div id="scale-up" class="module-page-container">
      <HeadlineAndResults
      ...
      <IntroConclusionModal
        ref="scaleUpConclusion"
        :titleAndText="scaleUpConclusion"
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      scaleUpConclusion: this.json.scaleUpConclusion,
  ...
  methods: {
    ...
    async checkIfDone() {
      if (!this.subActivitiesDone.includes(false)) {
        await genericAwait(1000)
        ;(this.$refs["scaleUpConclusion"] as any).show()
      }
    },
  ...
  computed: {
    ...
    showBadgesArr(): boolean[] {
      console.log("SHOW BADGES ARR IS")
      console.log(this.$store.getters["getBadgesShowing"])
