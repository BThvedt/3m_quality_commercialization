...
export const actions: ActionTree<MetaState, RootState> = {
  async loadMeta({ commit, dispatch, rootState, state }): Promise<void> {
    ...
    if (!noLms) {
      ...
      if (!unparsedSuspendedData) {
        ...
        suspendedData = jsonData.suspendedData as SuspendedData // ugh don't like this.. it's the currentRole as a string tha tmakes this needed, but not sure what to do
      } else {
        suspendedData = JSON.parse(unparsedSuspendedData)
      }
    } else {
      suspendedData = jsonData.suspendedData as SuspendedData // ugh don't like this.. it's the currentRole as a string that makes this needed, but not sure what to do
    }
    ...
    if (process.env.VUE_APP_RUN_MODE === "local_keyed") {
      ...
      await dispatch("editableText/loadLocalKeyedJson", null, { root: true })
    }
    ...
    if (noLms) {
      ...
      commit("setLastPageVisited", lastLocation)
  ...
  async setRole({ commit }, payload) {
    // console.log("INSIDE ACTION OF SET ROLE")
  ...
  async goForward({ commit, rootState, state, dispatch }): Promise<void> {
    ...
    if (currPage < state.sections[currSection].pages.length - 1) {
      ...
      const sectionId = state.sections[currSection].id
      const pageId = state.sections[currSection].pages[currPage + 1].id
      // for going back from 'hidden pages'
      commit("setLastPageVisited", `/${sectionId}/${pageId}`)
      ...
      if (!lockedArray[currSection + 1]) {
        ...
        commit("setLastLocation", `/${newSectionId}/${newPageId}`)
        commit("setLastPageVisited", `/${newSectionId}/${newPageId}`)
        if (!rootState.noLms) {
          console.log(
  ...
  async goHome({ state, dispatch, commit }) {
    const { currSection, currPage } = state
    const currRole = state.scorm.suspendedData.currentRole
    ...
    newPage = state.sections[newSection].pages.length - 1
  ...
  setModuleRawScore({ state, rootState, commit }, rawScore: number) {
    ...
    console.log("SETTING THE RAW SCORE")
  ...
  async goBackToRole({ dispatch, commit, rootState, state }): Promise<void> {
    commit("setRouteAction", RouteActions.GOING_TO_PREV_SECTION)

    commit("setAndGoToSectionAndPage", { sectionIndex: 0, pageIndex: 0 })
  },
  async goToPage(
  ...
  unlockSection({ state, commit, dispatch }, payload: number) {
    ...
    const { currentRole } = state.scorm.suspendedData
    const { lockedSections } = state.scorm.suspendedData
    ...
    console.log("Time to unlocak section")
    console.log(state.scorm.suspendedData)
  ...
  markSectionComplete({ state, commit, dispatch }, payload: number) {
    ...
    const sectionIndex = payload
    const { completedSections } = state.scorm.suspendedData
    const { currentRole } = state.scorm.suspendedData
    ...
    if (
      completedSections[currentRole].every(
        (sectionToComplete) => sectionToComplete
      )
    ) {
      dispatch("setModuleStatus", "passed")
      dispatch("setModuleRawScore", 100)
