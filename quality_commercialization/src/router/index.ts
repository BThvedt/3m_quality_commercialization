...
router.beforeEach((to, from, next) => {
  ...
  // if not in 'Product', reset the index
  if (store) {
    const sections = store.getters["meta/getAllSections"]

    // all paths have the form /section/page
    const splitPath = to.path.split("/")
    const sectionId = splitPath.length >= 2 ? splitPath[1] : ""

    if (sectionId) {
      const section = sections.find((section: Section) => {
        return section.id === sectionId
      })

      if (!section.name.includes("Product")) {
        // were not in the product section... make sure it's reset in the store
        store.dispatch("resetCommercializationLesson")
      }
    }
  }
